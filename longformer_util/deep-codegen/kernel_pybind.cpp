...
template <class T>
array1d_t<T> capsule_to_array1d(py::capsule& capsule) 
{
    ...
    T*  data_ptr = (T*)tensor->data;

    array1d_t<T> array(data_ptr, shape0);
    return array;
}

template <class T>
array2d_t<T> capsule_to_array2d(py::capsule& capsule) 
{
    DLManagedTensor * dlMTensor = static_cast<DLManagedTensor*>(capsule);
    assert(dlMTensor);
    DLTensor* tensor = &dlMTensor->dl_tensor;
    ...
    array2d_t<T> array(data_ptr, shape0, shape1);
    return array;
}

template <class T>
array3d_t<T> capsule_to_array3d(py::capsule& capsule) 
{
    DLManagedTensor * dlMTensor = static_cast<DLManagedTensor*>(capsule);
    assert(dlMTensor);
    DLTensor* tensor = &dlMTensor->dl_tensor;
    ...
    array3d_t<T> array(data_ptr, shape0, shape1, shape2);
    return array;
}

template <class T>
array4d_t<T> capsule_to_array4d(py::capsule& capsule) 
{
    DLManagedTensor * dlMTensor = static_cast<DLManagedTensor*>(capsule);
    assert(dlMTensor);
    DLTensor* tensor = &dlMTensor->dl_tensor;
    ...
    array4d_t<T> array(data_ptr, shape0, shape1, shape2, shape3);
    return array;
}
