...
#get array type
def get_type_name(array_dim_list):
    array_type_list = []
    for k in array_dim_list:
        array_type_list.append(k[0].split('>')[0].split('<')[-1])
...
def fuc_var_class(function_info):
    ...
    array_type_list = get_type_name(array_dim_list)
    array_index_list = [i for (i, item) in enumerate(array_dim_list) if 'array' in item[0]]
    return var_list, array_dim_list, array_index_list, array_type_list
...
def record_num_class(var_list, array_index_list, array_dim_list, array_type_list):
    output_list = []
    for (i, var_list_item) in enumerate(var_list):
        if i in array_index_list:
            ...
            temp1.append(array_type_list[i])
...
def generate_pybind_code(all_string):
    ...
    var_list, array_dim_list, array_index_list, array_type_list = fuc_var_class(fuc_var) #get initial function information
    output_list = record_num_class(var_list, array_index_list, array_dim_list, array_type_list) #get function args
