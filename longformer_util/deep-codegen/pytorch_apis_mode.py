...
class lformerMM_impl(th.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input1, input2, window, dilation, is_diagonal, autoregressive, mode):
        ...
        res = gp_apis.gp_lformerMM(input1, input2, dim1_0, dim1_1, dim1_2, dim1_3, dilation, no_dilation, window, 0 if autoregressive else window, 0 if mode != 2 else 1, device0)
...
def lformerMM(input1, input2, window, dilation, is_diagonal = False, autoregressive = False, mode = 3):
    return lformerMM_impl.apply(input1, input2, window, dilation, is_diagonal, autoregressive, mode)
